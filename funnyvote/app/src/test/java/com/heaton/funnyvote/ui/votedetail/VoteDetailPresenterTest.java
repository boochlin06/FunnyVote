...
public class VoteDetailPresenterTest {
    ...
    public void getVoteDataFromRepositoryAndLoadIntoView() {
        presenter = new VoteDetailPresenter(voteCode, voteDataRepository, userDataRepository
                , view, mSchedulerProvider);
        ...
        when(voteDataRepository.getOptions(voteData)).thenReturn(Observable.just(optionList));
        presenter.subscribe();
        verify(userDataRepository).getUser(eq(false));
        InOrder inOrder = Mockito.inOrder(view);
        inOrder.verify(view).showLoadingCircle();
        verify(voteDataRepository).getVoteData(eq(voteCode), eq(user));
        verify(view).setUpAdMob(eq(user));
        verify(voteDataRepository).getOptions(eq(voteData));
    ...
    public void searchOptionAndUpdateToView() {
        presenter = new VoteDetailPresenter(voteCode, voteDataRepository
                , userDataRepository, view, mSchedulerProvider);
    ...
    public void pollVoteAndUpdateToView() {
        ...
        when(voteDataRepository.getOptions(any())).thenReturn(Observable.just(optionList));
        presenter = new VoteDetailPresenter(voteCode, voteDataRepository
                , userDataRepository, view, mSchedulerProvider);
        presenter.pollVote("password");
        InOrder inOrder = Mockito.inOrder(view);
        verify(voteDataRepository).pollVote(anyString(), eq("password")
                , anyList(), any(User.class));
        inOrder.verify(view).showLoadingCircle();
        inOrder.verify(view).hideLoadingCircle();
    ...
    public void pollVoteNeedPWAndShakeDialogAfterShowPWDialog() {
        when(voteDataRepository.pollVote(anyString(), anyString(), anyList(), any(User.class)))
                .thenReturn(Observable.<VoteData>just(voteData));
        when(voteDataRepository.getOptions(any())).thenReturn(
    ...
    public void addNewOptionNeedPWAndShakeDialogAfterShowPWDialog() {
        when(voteDataRepository.addNewOption(anyString(), anyString(), anyList(), any())).thenReturn(
                Observable.error(new HttpException(
                        Response.error(500, ResponseBody.create(MediaType.parse("text/plain")
                                , "error_invalid_password")))));
        presenter = new VoteDetailPresenter(voteCode, voteDataRepository, userDataRepository
    ...
    @Test
    public void addNewOptionFailure() {
        ...
        presenter.addNewOptionCompleted("password", "newOptionText");
        InOrder inOrder = Mockito.inOrder(view);
        verify(voteDataRepository).addNewOption(anyString()
                , eq("password"), anyList(), any(User.class));
        inOrder.verify(view).showLoadingCircle();
        inOrder.verify(view).hideLoadingCircle();
    }
    ...
    public void clickOnOption_refreshOptionChoiceTest() {
        presenter = new VoteDetailPresenter(voteCode, voteDataRepository, userDataRepository, view, mSchedulerProvider);
